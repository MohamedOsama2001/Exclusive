    ...
    useEffect(()=>{
        const fetchFlashSaleProducts=async () =>{
            try{
                ...
                const flashSaleList=productsData.sort((a, b) => b.discountPercentage - a.discountPercentage).slice(0,4)
                const bestSellingList=productsData.sort((a,b)=>b.rating-a.rating).slice(0,4)
    ...
    useEffect(()=>{
        const fetchCategory=async ()=>{
            try{
                ...
                const categoriesList=response.data.slice(0,6)
                            ...
                            <div className="col" key={idx}>
                                <div className="card h-100 position-relative product-card" style={{ cursor: 'pointer' }}>
                                    <div className="position-relative" style={{ height: "250px" }}>
                                        <img src={item.images[0]} className="card-img-top mt-5 ms-5" alt="Product" />
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div>
                                            <span className="text-danger fs-5">{(item.price-(item.price*(item.discountPercentage/100))).toFixed(2)}$</span>
                                            <span className="text-muted text-decoration-line-through ms-2">{item.price}$</span>
                                        </div>
                                        <div className="mt-2">
                                            {[...Array(Math.round(item.rating || 0))].map((star, index) => (
                                                <span key={index} className="text-warning">&#9733;</span>
                                            ))}
                                            <span className="text-muted ms-2">({((item.rating/5)*100).toFixed(0)})</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="container my-5">
                    ...
                    <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
                        {bestSellingProducts.map((product)=>(
                            ...
                            <div className="card h-100 position-relative product-card" style={{ cursor: 'pointer' }}>
                                <div className="position-relative" style={{ height: "250px" }}>
                                    <img src={product.images[0]} className="card-img-top mt-5 ms-5" alt="Product" />
                                    <button className="btn btn-dark w-100 add-to-cart-btn" onClick={() => addToCart(product)}>{product.isInCart?"Remove From Cart":"Add To Cart"}</button>
                                </div>
                                <div className="position-absolute top-0 end-0 d-flex flex-column align-items-center p-2" style={{ zIndex: 1 }}>
                                    <span className="text-dark" style={{ cursor: 'pointer' }}>
                                        <i class="far fa-heart"></i>
                                ...
                                <div className="card-body">
                                    ...
                                    <div className="mt-2">
                                    {[...Array(Math.round(product.rating || 0))].map((star, index) => (
                                                <span key={index} className="text-warning">&#9733;</span>
                                            ))}
                                        <span className="text-muted ms-2">({((product.rating/5)*100).toFixed(0)})</span>
                                    </div>
                                </div>
                            </div>
                        </div>
